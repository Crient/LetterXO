...
export default async function handler(req, res) {
  ...
  const { data, error } = await supabase
    .from('valentines')
    .select(
      'id, sender_name, sender_email, receiver_name, receiver_email, letter_message, vibe, main_plan, food, place_text, place_pref, receiver_note, responded_at, created_at'
    )
    .eq('id', idResult.value)
    .eq('view_token', tokenResult.value)
    .single();

  if (error || !data) {
    return sendJson(res, 404, { error: 'Results not found.' });
  }
